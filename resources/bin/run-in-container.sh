...
function generate_node_conf {
  ...
  if [ $? -eq 0 ]; then
    echo "Generating node cluster configuration"
    ...
    echo "IP address cannot be determined, not generating node cluster configuration"
  fi
}
...
function generate_product_conf {
  ...
  if [ -e ${APP_HOME}/default-conf/xl-deploy.conf.template ]; then
    echo "Generating configuration file xl-deploy.conf from environment parameters"
    sed -e "s#\${XL_DB_DRIVER}#${XL_DB_DRIVER}#g" \
        -e "s#\${XL_CLUSTER_MODE}#${XL_CLUSTER_MODE}#g" \
        -e "s#\${XL_DB_URL}#${XL_DB_URL}#g" \
        -e "s#\${XL_DB_USERNAME}#${XL_DB_USERNAME}#g" \
  ...
  if [ -e ${APP_HOME}/default-conf/system.conf.template ]; then
    echo "Generating configuration file system.conf from environment parameters"
    sed -e "s#\${XL_DB_DRIVER}#${XL_DB_DRIVER}#g" \
        -e "s#\${XL_CLUSTER_MODE}#${XL_CLUSTER_MODE}#g" \
        -e "s#\${XL_DB_URL}#${XL_DB_URL}#g" \
        -e "s#\${XL_DB_USERNAME}#${XL_DB_USERNAME}#g" \
...
echo "*** This is the patched version of run-in-container.sh that should handle being started with a host network ***"
copy_db_driver
